...
#include <stdlib.h>
#include <time.h>
#include <conio.h>  // For Windows-specific functions like _kbhit() and _getch()
...
int main() {
    ...
    while (GameOn) {
        if (_kbhit()) { // Check if a key is pressed
            c = _getch(); // Get the pressed key
            ManipulateCurrent(c);
        }
        after = clock();
        if ((double)(after - before) > timer / CLOCKS_PER_SEC * 1000) { // Check if timer has passed
            before = after;
            ManipulateCurrent('s');
        }
    }
